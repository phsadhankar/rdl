...
class QDL::Util
  ...
  def self.to_class_str(cls)
    ...
    if cls_str.start_with? '#<Class:'
      cls_str = cls_str.split('(')[0] + '>' if cls_str['(']
      cls_str = QDL::Util.add_singleton_marker(cls_str[8..-2])
    end
    cls_str
  end
  ...
  # Returns the @__qdl_type field of [+obj+]
  def self.qdl_type(obj)
    return (obj.instance_variable_defined?('@__qdl_type') && obj.instance_variable_get('@__qdl_type'))
  end

  def self.qdl_type_or_class(obj)
    return self.qdl_type(obj) || obj.class
  end
