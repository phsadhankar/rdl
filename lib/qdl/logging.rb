class QDL::Logging
  ...
  def self.log_header(area, message_level, header)
    ...
    log_level = QDL::Config.instance.log_levels[area] || :info
    tracing = log_level == :trace

    return unless log_level_leq(log_level, message_level)

    if QDL::Config.instance.disable_log_colors
  ...
  def self.log_header_to_file(area, message_level, header)
    return unless QDL::Config.instance.log_file

    header = header.to_s
    log_level = QDL::Config.instance.log_file_levels[area] || :info
    ...
    File.open(QDL::Config.instance.log_file, "a+") { |f| f.puts no_color_str }
  end
  ...
  def self.log(area, message_level, message, ast: nil)
    ...
    no_color = QDL::Config.instance.disable_log_colors
  ...
  def self.log_message_to_file(area, message_level, message, ast)
    ...
    File.open(QDL::Config.instance.log_file, "a+") do |f|
      f.puts str
