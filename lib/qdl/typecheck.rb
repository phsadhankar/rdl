...
module QDL::Typecheck
  ...
  class Env
    ...
    def merge_block_env(other, arg_names)
      ...
      @env.each { |k, v|
        if (other.env.has_key? k) && (not (arg_names.include? k))
          ...
          typ = QDL::Type::UnionType.new(v[:type], other.env[k][:type])
          typ = typ.canonical
          if typ.instance_of?(QDL::Type::UnionType)
            sings = 0
            typ.types.each { |t| sings = sings + 1 if t.instance_of?(QDL::Type::SingletonType) }
            typ = typ.widen if sings > QDL::Config.instance.widen_bound
          end
    ...
    def self.join(e, *envs)
      ...
      first.env.each_pair { |var, h|
        first_typ = h[:type]
        if h[:fixed]
          QDL::Typecheck.error :inconsistent_var_type, [var.to_s], e unless rest.all? { |other| (other.fixed? var) || (not (other.has_key? var)) }
          neq = []
          rest.each { |other|
            other_typ = other[var]
            neq << other_typ unless first_typ == other_typ
          }
          QDL::Typecheck.error :inconsistent_var_type_type, [var.to_s, ([first_typ] + neq).map { |t| t.to_s }.join(' and ')], e unless neq.empty?
          env.env[var] = {type: h[:type], fixed: true}
        else
          typ = QDL::Type::UnionType.new(first_typ, *rest.map { |other| ((other.has_key? var) && other[var]) || QDL::Globals.types[:nil] })
          typ = typ.canonical
          if typ.instance_of?(QDL::Type::UnionType)
            sings = 0
            typ.types.each { |t| sings = sings + 1 if t.instance_of?(QDL::Type::SingletonType) }
  ...
  def self.is_QDL(node)
    return node != nil && node.type == :const && node.children[0] == nil && node.children[1] == :QDL
  end

  def self.get_ast(klass, meth)
    file, line = QDL::Globals.info.get(klass, meth, :source_location)

    return nil if file.nil?
    #   return nil if QDL::Config.instance.continue_on_errors
    #
    #   raise RuntimeError, "No file for #{QDL::Util.pp_klass_method(klass, meth)}" if file.nil?
    # end

    raise RuntimeError, "static type checking in irb not supported" if file == "(irb)"
    if file == "(pry)"
      # no caching...
      if QDL::Wrap.wrapped?(klass, meth)
        meth_name = QDL::Wrap.wrapped_name(klass, meth)
      else
        meth_name = meth
      end
      the_meth = QDL::Util.to_class(klass).instance_method(meth_name)
      code = Pry::Code.from_method the_meth
      return Parser::CurrentRuby.parse code.to_s
    end

    digest = Digest::MD5.file file
    cache_hit = ((QDL::Globals.parser_cache.has_key? file) &&
                 (QDL::Globals.parser_cache[file][0] == digest))
    ...
    unless cache_hit
      begin
        ...
        QDL::Globals.parser_cache[file] = [digest, cache]
      rescue => e
        QDL::Logging.log :typecheck, :error, "Failed to parse #{file}; #{e}"
        return nil if QDL::Config.instance.continue_on_errors

        raise e
      end
    end
    ast = QDL::Globals.parser_cache[file][1][:line_defs][line]
    return ast
  end
  ...
  def self.infer(klass, meth)
    ...
    raise exn unless QDL::Config.instance.continue_on_errors
    QDL::Logging.log :inference, :error, "Error; Skipping inference for #{QDL::Util.pp_klass_method(klass, meth)}"
    QDL::Logging.log :inference, :debug, "... got exception: #{exn}"
    # QDL::Globals.info.set(klass, meth, :type, [QDL::Globals.types[:dyn]])
  end

  def self._infer(klass, meth)
    QDL::Logging.log_header :inference, :debug, "Infering #{QDL::Util.pp_klass_method(klass, meth)}"

    QDL::Config.instance.use_comp_types = true
    QDL::Config.instance.number_mode = true
    @var_cache = {}
    ast = get_ast(klass, meth)
    if ast.nil?
      QDL::Logging.log :inference, :warning, "Warning: Can't find source for class #{QDL::Util.pp_klass_method(klass, meth)}; skipping method"

      # if QDL::Config.instance.continue_on_errors
      #   puts "#{warning_text} recording %dyn" if QDL::Config.instance.convert_to_dyn_verbose
      #   QDL::Globals.info.set(klass, meth, :type, [QDL::Globals.types[:dyn]])
      #   return
      # end
    ...
    if name.to_sym != meth
      QDL::Logging.log :inference, :info, "Aliasing #{meth} to #{name}"
      QDL.alias klass, meth, name
      return
    end
    error :internal, "Method #{name} defined where method #{meth} expected", ast if name.to_sym != meth
    context_types = QDL::Globals.info.get(klass, meth, :context_types)
    ...
    if QDL::Util.has_singleton_marker(klass)
      ...
      self_type = QDL::Type::SingletonType.new(QDL::Util.to_class(QDL::Util.remove_singleton_marker(klass)))
    else
      self_type = QDL::Type::NominalType.new(klass)
    end
    ...
    if body.nil?
      body_type = QDL::Globals.types[:nil]
    else
    ...
    body_type = QDL::Globals.parser.scan_str "#T self" if meth == :initialize # JF: Why not inst.self?
    if body_type.is_a?(QDL::Type::UnionType)
      body_type.types.each { |t| QDL::Type::Type.leq(t, ret_vartype, ast: ast) }
    else
      QDL::Type::Type.leq(body_type, ret_vartype, ast: ast)
    end

    QDL::Globals.info.set(klass, meth, :typechecked, true)

    QDL::Globals.constrained_types << [klass, meth]
    QDL::Logging.log :inference, :debug, "Done with constraint generation."
  end

  def self.typecheck(klass, meth, ast=nil, types = nil)
    # puts "Typechecking #{klass}##{meth}"
    ast = get_ast(klass, meth) unless ast
    raise RuntimeError, "Can't find source for class #{QDL::Util.pp_klass_method(klass, meth)}" if ast.nil?
    types = QDL::Globals.info.get(klass, meth, :type) unless types
    ...
    types.each { |type|
      ...
      if meth == :initialize
        ...
        error :bad_initialize_type, [], ast unless ((type.ret.is_a?(QDL::Type::VarType) && type.ret.name == :self) || (type.ret.is_a?(QDL::Type::GenericType) && type.ret.base.is_a?(QDL::Type::VarType) && type.ret.base.name == :self))
      end
      raise RuntimeError, "Type checking of methods with computed types is not currently supported." unless (type.args + [type.ret]).all? { |t| !t.instance_of?(QDL::Type::ComputedType) }
      inst = {self: self_type}
      ...
      error :bad_return_type, [body_type.to_s, type.ret.to_s], body unless body.nil? || meth == :initialize ||QDL::Type::Type.leq(body_type, type.ret, ast: ast)
    }

    if QDL::Config.instance.check_comp_types
      new_meth = WrapCall.rewrite(ast) # rewrite ast to insert dynamic checks
      QDL::Util.silent_warnings { QDL::Util.to_class(klass).class_eval(new_meth) } # redefine method in the same class
    end
    QDL::Globals.info.set(klass, meth, :typechecked, true)
  end
      ...
      hash.each { |k, v|
        ...
        when QDL::Type::PreciseStringType
          if v.vals.size > 10 || (v.vals.size == 1 && v.vals[0].size > 10)
            newhash[k] = QDL::Globals.types[:string]
          else
            newhash[k] = v
          end
  ...
  def self.args_hash(scope, env, type, args, ast, kind)
    ...
    args.children.each_with_index { |arg, index|
      ...
      targ = type.args[tpos]
      (if (targ.is_a?(QDL::Type::AnnotatedArgType) || targ.is_a?(QDL::Type::DependentArgType) || targ.is_a?(QDL::Type::BoundArgType)) then targ = targ.type end)
      if arg.type == :arg
        if kind == 'block'
        ...
        error :optional_default_type, [default_type, targ.type], arg.children[1], block: (kind == 'block') unless QDL::Type::Type.leq(default_type, targ.type, ast: ast)
        ...
        targs[arg.children[0]] = QDL::Type::GenericType.new(QDL::Globals.types[:array], targ.type)
        tpos += 1
      elsif arg.type == :kwarg
        error :type_args_no_kws, [kind], arg, block: (kind == 'block') unless targ.is_a?(QDL::Type::FiniteHashType)
        kw = arg.children[0]
        error :type_args_no_kw, [kind, kw], arg, block: (kind == 'block') unless targ.elts.has_key? kw
        tkw = targ.elts[kw]
        error :type_args_kw_mismatch, [kind, 'optional', kw, 'required'], arg, block: (kind == 'block') if tkw.is_a? QDL::Type::OptionalType
        kw_args_matched << kw
        targs[kw] = tkw
        ...
        error :type_args_no_kw, [kind, kw], arg, block: (kind == 'block') unless targ.elts.has_key? kw
        tkw = targ.elts[kw]
        error :type_args_kw_mismatch, [kind, 'required', kw, 'optional'], arg, block: (kind == 'block') if !tkw.is_a?(QDL::Type::OptionalType)
        env, default_type = tc(scope, env, arg.children[1])
        error :optional_default_kw_type, [kw, default_type, tkw.type], arg.children[1], block: (kind == 'block') unless QDL::Type::Type.leq(default_type, tkw.type, ast: ast)
        kw_args_matched << kw
        targs[kw] = tkw.type
        env = env.merge(Env.new(kw => tkw.type))
      elsif arg.type == :kwrestarg
        error :type_args_no_kws, [kind], e, block: (kind == 'block') unless targ.is_a?(QDL::Type::FiniteHashType)
        error :type_args_no_kw_rest, [kind], arg, block: (kind == 'block') if targ.rest.nil?
        targs[arg.children[0]] = QDL::Type::GenericType.new(QDL::Globals.types[:hash], QDL::Globals.types[:symbol], targ.rest)
    ...
    if (tpos == type.args.length - 1) && type.args[tpos].is_a?(QDL::Type::FiniteHashType)
      ...
      unless (type.args.length == 1) && (type.args[0].is_a?(QDL::Type::OptionalType) || type.args[0].is_a?(QDL::Type::VarargType)) && args.children.empty?
  ...
  def self.get_super_owner(slf, m)
    ...
    when QDL::Type::SingletonType
      if slf.nominal.name == 'Class'
        ...
        QDL::Type::SingletonType.new(QDL::Util.singleton_class_to_class(trecv_owner))
      else
        raise Exception, "self is singleton class but nominal is not Class, it is #{slf.nominal.name}"
      end
    when QDL::Type::NominalType
      QDL::Type::NominalType.new(get_super_owner_from_class(slf.klass, m))
    else
  ...
  def self.tc(scope, env, e)
    ...
    QDL::Logging.log :typecheck, :debug_error, "#{exn}; returning %dyn"

    [env, QDL::Globals.types[:dyn]]
  end
  ...
  def self._tc(scope, env, e)
    case e.type
    when :nil
      [env, QDL::Globals.types[:nil]]
    when :true
      [env, QDL::Globals.types[:true]]
    when :false
      [env, QDL::Globals.types[:false]]
    when :str, :string
      t = QDL::Config.instance.use_precise_string ? QDL::Type::PreciseStringType.new(e.children[0]) : QDL::Globals.types[:string]
      [env, t]
    when :complex, :rational # constants
      [env, QDL::Type::NominalType.new(e.children[0].class)]
    when :int, :float
      if QDL::Config.instance.number_mode
        [env, QDL::Type::NominalType.new(Integer)]
      else
        [env, QDL::Type::SingletonType.new(e.children[0])]
      end
    when :sym # singleton
      [env, QDL::Type::SingletonType.new(e.children[0])]
    when :dstr, :xstr # string (or execute-string) with interpolation
      ...
      e.children.each { |ei|
        ...
        if QDL::Config.instance.use_precise_string
          if ei.type == :str || ei.type == :string
            ...
            prec_str << (if ti.is_a?(QDL::Type::SingletonType) then ti.val.to_s else ti end)
          end
          t = QDL::Type::PreciseStringType.new(*prec_str)
        else
          t = QDL::Globals.types[:string]
        end
      }
      [envi, t]
    ...
    when :array
      ...
      e.children.each { |ei|
        if ei.type == :splat
          ...
          if ti.is_a? QDL::Type::TupleType
            ti.cant_promote! # must remain a tuple
            tis.concat(ti.params)
          elsif ti.is_a? QDL::Type::FiniteHashType
            ti.cant_promote! # must remain a finite hash
            ti.elts.each_pair { |k, t|
              tis << QDL::Type::TupleType.new(QDL::Type::SingletonType.new(k), t)
            }
          elsif ti.is_a?(QDL::Type::GenericType) && ti.base == QDL::Globals.types[:array]
            is_array = true
            tis << ti.params[0]
          elsif ti.is_a?(QDL::Type::GenericType) && ti.base == QDL::Globals.types[:hash]
            is_array = true
            tis << QDL::Type::TupleType.new(*ti.params)
          elsif ti.is_a?(QDL::Type::SingletonType) && ti.val.nil?
            # nil gets thrown out
          elsif ti.is_a?(QDL::Type::UnionType) && ti.types.all? { |t| t.is_a?(QDL::Type::GenericType) && t.base == QDL::Globals.types[:array] }
            is_array = true
            tis << QDL::Type::UnionType.new(*ti.types.map { |t| t.params[0] }).canonical
          elsif ti.is_a?(QDL::Type::VarType)
              new_arr_type = QDL::Type::GenericType.new(QDL::Globals.types[:array], v = make_unknown_var_type(ti, :splat_param, "splat param"))
              QDL::Type::Type.leq(ti, new_arr_type, ast: e)
              tis << v
          elsif QDL::Type::Type.leq(QDL::Globals.types[:array], ti, ast: e) || QDL::Type::Type.leq(ti, QDL::Globals.types[:array], ast: e) ||
                QDL::Type::Type.leq(QDL::Globals.types[:hash], ti, ast: e) || QDL::Type::Type.leq(ti, QDL::Globals.types[:hash], ast: e)
      ...
      if is_array
        [envi, QDL::Type::GenericType.new(QDL::Globals.types[:array], QDL::Type::UnionType.new(*tis).canonical)]
      elsif scope[:task] == :infer && QDL::Config.instance.infer_empties && tis.empty?
        [envi, QDL::Type::GenericType.new(QDL::Globals.types[:array], QDL::Type::VarType.new(cls: scope[:klass], meth: scope[:meth], category: :array_param, name: "array_param_#{e.loc}"))]
      else
        [envi, QDL::Type::TupleType.new(*tis)]
      end
    when :hash
      envi = env
      tlefts = []
      ...
      e.children.each { |p|
        ...
        if p.type == :pair
          ...
          trights << tright
          is_fh = false unless tleft.is_a?(QDL::Type::SingletonType)
        elsif p.type == :kwsplat
          envi, tkwsplat = tc(scope, envi, p.children[0])

          if tkwsplat.is_a? QDL::Type::FiniteHashType
            tkwsplat.cant_promote! # must remain finite hash
            tlefts.concat(tkwsplat.elts.keys.map { |k| QDL::Type::SingletonType.new(k) })
            trights.concat(tkwsplat.elts.values)
          elsif tkwsplat.is_a?(QDL::Type::GenericType) && tkwsplat.base == QDL::Globals.types[:hash]
            is_fh = false
      ...
      if is_fh
        ...
        if scope[:task] == :infer && QDL::Config.instance.infer_empties && fh.empty?
          [envi, QDL::Type::GenericType.new(QDL::Globals.types[:hash], QDL::Type::VarType.new(cls: scope[:klass], meth: scope[:meth], category: :hash_param_key, name: "hash_param_key_#{e.loc}"),
                                            QDL::Type::VarType.new(cls: scope[:klass], meth: scope[:meth], category: :hash_param_val, name: "hash_param_val_#{e.loc}"))]
        else
          [envi, QDL::Type::FiniteHashType.new(fh, nil)]
        end
      else
        tleft = QDL::Type::UnionType.new(*tlefts)
        tright = QDL::Type::UnionType.new(*trights)
        [envi, QDL::Type::GenericType.new(QDL::Globals.types[:hash], tleft, tright)]
      end
      #TODO test!
#    when :kwsplat # TODO!
    when :irange, :erange
      ...
      t1 = QDL::Type::NominalType.new(t1.val.class) if t1.is_a? QDL::Type::SingletonType
      t2 = QDL::Type::NominalType.new(t2.val.class) if t2.is_a? QDL::Type::SingletonType
      error :nonmatching_range_type, [t1, t2], e unless t1 <= t2 || t2 <= t1
      [env2, QDL::Type::GenericType.new(QDL::Globals.types[:range], t1)]
    when :self
      [env, env[:self]]
    ...
    when :lvasgn, :ivasgn, :cvasgn, :gvasgn
      ...
      env = env.bind(x, QDL::Globals.types[:nil]) if ((e.type == :lvasgn) && (not (env.has_key? x)))
    ...
    when :masgn
      ...
      e.children[0].children.each { |asgn|
        ...
        x = e.children[0]
        env = env.bind(x, QDL::Globals.types[:nil]) if (not (env.has_key? x)) # see lvasgn
      ...
      if tright.is_a? QDL::Type::TupleType
        ...
        if splat_ind
          ...
          splat = lhs[splat_ind]
          envi, _ = tc_vasgn(scope, envi, splat.children[0].type, splat.children[0].children[0], QDL::Type::TupleType.new(*rhs), splat) if splat.children.any?
          [envi, tright]
        else
      ...
      elsif (tright.is_a? QDL::Type::GenericType) && (tright.base == QDL::Globals.types[:array])
        ...
        [envi, tright]
      elsif (tright.is_a? QDL::Type::DynamicType)
        tasgn = tright
        lhs.each { |asgn|
          if asgn.type == :splat
            envi, _ = tc_vasgn(scope, envi, asgn.children[0].type, asgn.children[0].children[0], tright, asgn)
        ...
        [env, tright]
      elsif tright.is_a?(QDL::Type::VarType)
        splat_ind = lhs.index { |lhs_elt| lhs_elt.type == :splat }
        ...
        if splat_ind
          ...
          if splat_ind > 0
            lhs[0..splat_ind-1].each { |left|
              # before splat
              envi, _ = tc_vasgn(scope, envi, left.type, left.children[0], QDL::Type::VarType.new(cls: scope[:klass], meth: scope[:meth], category: :tuple_element, name: "tuple_element_#{count}"), left)
              count += 1
            }
          end
          lhs[splat_ind+1..-1].reverse_each { |left|
          ...
          splat = lhs[splat_ind]
          envi, _ = tc_vasgn(scope, envi, splat.children[0].type, splat.children[0].children[0], QDL::Type::GenericType.new(QDL::Globals.types[:array], QDL::Type::VarType.new(cls: scope[:klass], meth: scope[:meth], category: :tuple_element, name: "array_param_#{count}")), splat) if splat.children.any? ## could be empty splat
          [envi, tright]
        else
          new_tuple = []
          count = 0
          lhs.length.times { new_tuple << QDL::Type::VarType.new(cls: scope[:klass], meth: scope[:meth], category: :tuple_element, name: "tuple_element_#{count}") }
          lhs.zip(new_tuple).each { |left, right|
            envi, _ = tc_vasgn(scope, envi, left.type, left.children[0], right, left)
          }
          #tuple_type = QDL::Type::TupleType.new(*new_tuple)
          #QDL::Type::Type.leq(tright, tuple_type, ast: e) # don't think this is necessary
          [envi, tright]
        end
      else
    ...
    when :op_asgn
      if e.children[0].type == :send
        ...
        env = env.bind(x, QDL::Globals.types[:nil]) if ((e.children[0].type == :lvasgn) && (not (env.has_key? x))) # see :lvasgn
    ...
    when :and_asgn, :or_asgn
      ...
      envi, trhs = (if tleft.is_a? QDL::Type::SingletonType
                      if e.type == :and_asgn
                      ...
                      if trecv.is_a?(QDL::Type::VarType)
                        ...
                        [Env.join(e, envleft, envright), QDL::Type::UnionType.new(tleft, tright).canonical]
                      end
                    end)
      if e.children[0].type == :send
        mutation_meth = (meth.to_s + '=').to_sym
    ...
    when :match_with_lvasgn # /regexp/ =~ rhs
      ...
      es, ts = tc_send(scope, env2, QDL::Globals.types[:regexp], :=~, [t2], nil, e)
      [es, ts]
      # (regexp
      #   (str "foo")
      #   (regopt))
    ...
    when :nth_ref, :back_ref
      [env, QDL::Globals.types[:string]]
    when :const
      if e.children[0].nil? && e.children[1] == :ENV
        ## ENV
        c = QDL::Type::GenericType.new(QDL::Globals.types[:hash], QDL::Globals.types[:string], QDL::Globals.types[:string])
      elsif e.children[0].nil? && e.children[1] == :ARGV
        c = QDL::Type::GenericType.new(QDL::Globals.types[:array], QDL::Globals.types[:string])
      else
    ...
    when :send, :csend
      ...
      return tc_var_type(scope, env, e) if (e.children[0].nil? || is_QDL(e.children[0])) && e.children[1] == :var_type
      return tc_type_cast(scope, env, e) if is_QDL(e.children[0]) && e.children[1] == :type_cast && scope[:block].nil?
      return tc_note_type(scope, env, e)  if is_QDL(e.children[0]) && e.children[1] == :note_type
      return tc_instantiate!(scope, env, e)  if is_QDL(e.children[0]) && e.children[1] == :instantiate!
      envi = env
      tactuals = []
      block = scope[:block]
      map_case = false
      ...
      scope_merge(scope, block: nil, break: env, next: env) { |sscope|
        e.children[2..-1].each { |ei|
          if ei.type == :splat
            ...
            if ti.is_a? QDL::Type::TupleType
              ...
              tactuals << QDL::Type::VarargType.new(ti.params[0]) # Turn Array<t> into *t
            elsif ti.is_a?(QDL::Type::VarType)
              v = make_unknown_var_type(ti, :splat_param, "splat param")
              #new_arr_type = QDL::Type::GenericType.new(QDL::Globals.types[:array], v = make_unknown_var_type(ti, :splat_param, "splat param"))
              #QDL::Type::Type.leq(ti, new_arr_type, ast: e)
              tactuals << QDL::Type::VarargType.new(v)
              #tactuals << QDL::Type::VarargType.new(QDL::Globals.types[:top])
            elsif ti.is_a?(QDL::Type::UnionType) && ti.types.all? { |t| t.is_a?(QDL::Type::GenericType) && t.base == QDL::Globals.types[:array] }
              params_union = QDL::Type::UnionType.new(*ti.types.map { |t| t.params[0] }).canonical
              tactuals << QDL::Type::VarargType.new(params_union)
            elsif ti.is_a?(QDL::Type::UnionType) && ti.types.any? { |t| t.is_a?(QDL::Type::TupleType) || (t.is_a?(QDL::Type::GenericType) && t.base == QDL::Globals.types[:array]) }
              raise "TODO"
            else
            ...
            if (e.children[1] == :map) || e.children[1] == :sum || e.children[1] == :keep_if
              ...
              envi, ti = tc_send(sscope, envi, ti, :to_proc, [], nil, ei) unless ti.is_a? QDL::Type::MethodType
              block = [ti, ei]
            end
          else
        ...
        if map_case && trecv.is_a?(QDL::Type::GenericType)
          #raise "Expected GenericType, got #{trecv}." unless trecv.is_a?(QDL::Type::GenericType)
          trecv.is_a?(QDL::Type::GenericType)
          _, ti_map_case = tc_send(sscope, Env.new({ self: trecv.params[0] }), ti_map_case, :to_proc, [], nil, e_map_case)
          map_block_type = QDL::Type::MethodType.new([trecv.params[0]], nil, ti_map_case.canonical.ret)
    ...
    when :yield
      ...
      error :block_block, [], e if scope[:tblock].is_a?(QDL::Type::MethodType) && scope[:tblock].block
      ...
      if scope[:tblock].is_a?(QDL::Type::VarType)
        block_ret_type = QDL::Type::VarType.new(cls: scope[:klass], meth: scope[:meth], category: :block_ret, name: "block_return")
        block_type = QDL::Type::MethodType.new(tactuals, nil, block_ret_type)
        QDL::Type::Type.leq(scope[:tblock], block_type, ast: e)
...
RUBY
    ...
    when :and, :or
      ...
      envright, tright = tc(scope, envleft, e.children[1])
      if tleft.is_a? QDL::Type::SingletonType
        if e.type == :and
          if tleft.val then [envright, tright] else [envleft, tleft] end
        else # e.type == :or
          if tleft.val then [envleft, tleft] else [envright, tright] end
        end
      elsif e.type == :and && !(tleft == QDL::Globals.types[:bool] || tleft == QDL::Globals.types[:false] || tleft.is_a?(QDL::Type::VarType))
        ...
        [envleft.merge(envright), QDL::Type::UnionType.new(tleft, tright).canonical]
      end
    ...
    #   if t1.is_a? QDL::Type::SingletonType
    #     if t1.val then [a1, QDL::Globals.types[:false]] else [a1, QDL::Globals.types[:true]] end
    #   else
    #     [a1, QDL::Globals.types[:bool]]
    #   end
    when :if
      envi, tguard = tc(scope, env, e.children[0]) # guard; any type allowed
      # always type check both sides
      envleft, tleft = if e.children[1].nil? then [envi, QDL::Globals.types[:nil]] else tc(scope, envi, e.children[1]) end # then
      envright, tright = if e.children[2].nil? then [envi, QDL::Globals.types[:nil]] else tc(scope, envi, e.children[2]) end # else
      if tguard.is_a? QDL::Type::SingletonType
    ...
    when :case
      ...
      e.children[1..-2].each { |wclause|
        ...
        if (tguards.all? { |typ| typ.is_a?(QDL::Type::SingletonType) && (typ.val.is_a?(Class) || typ.val.nil?) }) && (e.children[0].type == :lvar)
          ...
          new_typ = QDL::Type::UnionType.new(*(tguards.map { |typ| typ.val.nil? ? typ : QDL::Type::NominalType.new(typ.val) })).canonical
          # TODO adjust following for generics!
          if tcontrol.is_a? QDL::Type::GenericType
            ...
            next unless tcontrol.is_a?(QDL::Type::VarType) || (tcontrol <= new_typ || new_typ <= tcontrol) # If control can't possibly match type, skip this branch
      ...
      [Env.join(e, *envbodies), QDL::Type::UnionType.new(*tbodies).canonical]
    when :while, :until
      ...
      scope_merge(scope, break: env_break, tbreak: QDL::Globals.types[:nil], next: env, redo: env_break) { |lscope|
        begin
    ...
    when :while_post, :until_post
      ...
      scope_merge(scope, break: nil, tbreak: QDL::Globals.types[:nil], next: nil, redo: nil) { |lscope|
        if e.children[1]
    ...
    when :for
      ...
      when QDL::Type::NominalType
        ...
        teaches = filter_comp_types(teaches, QDL::Config.instance.use_comp_types)
      when QDL::Type::GenericType, QDL::Type::TupleType, QDL::Type::FiniteHashType, QDL::Type::PreciseStringType
        unless tcollect.is_a? QDL::Type::GenericType
          error :tuple_finite_hash_promote, (if tcollect.is_a? QDL::Type::TupleType then ['tuple', 'Array'] elsif tcollect.is_a? QDL::Type::PreciseStringType then ['precise string', 'String'] else ['finite hash', 'Hash'] end), e.children[1] unless tcollect.promote!
          tcollect = tcollect.canonical
        end
        ...
        teaches = teaches.map { |typ|
          ...
          if (typ.args+[typ.ret]+block_types).all? { |t| !t.instance_of?(QDL::Type::ComputedType) }
            typ
          else
    ...
    when :break, :redo, :next, :retry
      ...
      if e.children[0]
        ...
        scope[tkw_name] = QDL::Type::UnionType.new(scope[tkw_name], tkw)
      end
      scope[e.type] = Env.join(e, scope[e.type], env)
      [env, QDL::Globals.types[:bot]]
    when :return
      # TODO return in lambda returns from lambda and not outer scope
      if e.children[0]
         env1, t1 = tc(scope, env, e.children[0])
      else
         env1, t1 = [env, QDL::Globals.types[:nil], [:+, :+]]
      end
      error :bad_return_type, [t1.to_s, scope[:tret]], e unless QDL::Type::Type.leq(t1, scope[:tret], ast: e)
      [env1, QDL::Globals.types[:bot]] # return is a void value expression
    ...
    when :rescue
      ...
      scope_merge(scope, retry: env, exn: nil) { |rscope|
        ...
        [Env.join(e, *env_res), QDL::Type::UnionType.new(*tres).canonical]
      }
    when :resbody
      ...
      if e.children[0]
        e.children[0].children.each { |exn|
          ...
          error :exn_type, [], exn unless texn.is_a?(QDL::Type::SingletonType) && (texn.val.is_a?(Class) || texn.val.is_a?(Module))
          texns << QDL::Type::NominalType.new(texn.val)
        }
      else
        texns = [QDL::Globals.types[:standard_error]]
      end
      if e.children[1]
        envi, _ = tc_vasgn(scope, envi, :lvasgn, e.children[1].children[0], QDL::Type::UnionType.new(*texns), e.children[1])
      end
      env_fin, t_fin = if e.children[2].nil? then [envi, QDL::Globals.types[:nil]] else tc(scope, envi, e.children[2]) end
      [env_fin, t_fin]
    when :super
      envi = env
      tactuals = []
...
=end
      klass = QDL::Util.to_class scope[:klass]
      mname = scope[:meth]
      sklass = get_super_owner_from_class klass, mname
      sklass_str = QDL::Util.to_class_str sklass
      stype = lookup(scope, sklass_str, mname, e)#QDL::Globals.info.get_with_aliases(sklass_str, mname, :type)
      error :no_instance_method_type, [sklass_str, mname], e unless stype
      stype = filter_comp_types(stype, QDL::Config.instance.use_comp_types)
  ...
  def self.to_type(val, as_key=false)
    case val
    when Symbol
      if as_key then val else QDL::Type::SingletonType.new(val) end
    when TrueClass, FalseClass, Class, Module
      QDL::Type::SingletonType.new(val)
    when Complex, Rational, Integer, Float
      if QDL::Config.instance.number_mode# && !(val.class == Integer)
  ...
  def self.tc_var(scope, env, kind, name, e)
    ...
    when :ivar, :cvar, :gvar
      klass = (if kind == :gvar then QDL::Util::GLOBAL_NAME else env[:self].to_s end)
      klass = "Integer" if klass == "Number"
      klass = QDL::Util.remove_singleton_marker klass if QDL::Util.has_singleton_marker klass
      if QDL::Globals.info.has?(klass, name, :type)
        type = QDL::Globals.info.get(klass, name, :type)
      elsif QDL::Config.instance.assume_dyn_type
        type = QDL::Globals.types[:dyn]
      elsif QDL::Config.instance.use_unknown_types || QDL::Globals.to_infer.values.any? { |set| set.include?([klass, name]) }
  ...
  def self.tc_vasgn(scope, env, kind, name, tright, e)
    ...
    when :lvasgn
      if (env.fixed? name)
        error :vasgn_incompat, [tright, env[name]], e unless QDL::Type::Type.leq(tright, env[name], inst={}, true, ast: e)
    ...
    when :ivasgn, :cvasgn, :gvasgn
      klass = (if kind == :gvasgn then QDL::Util::GLOBAL_NAME else env[:self].to_s end)
      klass = "Integer" if klass == "Number"
      klass = QDL::Util.remove_singleton_marker klass if QDL::Util.has_singleton_marker klass
      if QDL::Globals.info.has?(klass, name, :type)
        tleft = QDL::Globals.info.get(klass, name, :type)
      elsif QDL::Config.instance.assume_dyn_type
        tleft = QDL::Globals.types[:dyn]
      elsif QDL::Config.instance.use_unknown_types || QDL::Globals.to_infer.values.any? { |set| set.include?([klass, name]) }
        tleft = make_unknown_var_type(klass, name, :var)
      else
        error :untyped_var, [kind_text, name, klass], e
      end
      error :vasgn_incompat, [tright.to_s, tleft.to_s], e unless QDL::Type::Type.leq(tright, tleft, inst={}, true, ast: e)
      tright.instantiate(inst)
      [env, tright.canonical]
    when :send
  ...
  def self.tc_var_type(scope, env, e)
    ...
    begin
      typ = QDL::Globals.parser.scan_str("#T " + typ_str)
    rescue Racc::ParseError => err
      error :generic_error, [err.to_s[1..-1]], e.children[3] # remove initial newline
    end
    [env.bind(var, typ, fixed: true), QDL::Globals.types[:nil]]
  end
  ...
  def self.tc_type_cast(scope, env, e)
    ...
    begin
      if QDL::Util.has_singleton_marker(typ_str)
        typ = QDL::Type::SingletonType.new(QDL::Globals.parser.scan_str("#T #{QDL::Util.remove_singleton_marker(typ_str)}").klass)
      else
        typ = QDL::Globals.parser.scan_str("#T " + typ_str)
      end
    rescue Racc::ParseError => err
  ...
  def self.tc_instantiate!(scope, env, e)
    ...
    formals, _, _ = QDL::Globals.type_params[klass]
    ...
    typ_args.each { |a|
      ...
      when QDL::Type::SingletonType
        ...
        new_typs << QDL::Globals.parser.scan_str("#T #{arg_typ.val}")
      else
        error :instantiate_format, [], a unless (a.type == :str) || (a.type == :string) || (a.type == :sym)
        new_typs << QDL::Globals.parser.scan_str("#T #{a.children[0]}")
      end
    }
  ...
  def self.tc_send(scope, env, trecvs, meth, tactuals, block, e, op_asgn=false)
    ...
    trecvs = if trecvs.is_a? QDL::Type::UnionType then union = true; trecvs.types else union = false; [trecvs] end

    trets = []
    trecvs.each { |trecv|
      if trecv.is_a?(QDL::Type::ChoiceType)
        choice_hash = { }
        errs = []
        trecv.choices.each { |num, t|
          ...
          begin
            t = t.canonical
            tarms = t.is_a?(QDL::Type::UnionType) ? t.types : [t]
            tarms.each { |t|
              env, ts = tc_send_one_recv(scope, env, t, meth, tactuals, block, e, op_asgn, union)
              choice_hash[num] = QDL::Type::UnionType.new(*ts).canonical
            }
        ...
        if choice_hash.empty?
          ...
          ts = [QDL::Type::ChoiceType.new(choice_hash, [trecv] + trecv.connecteds)]
        end
      else
    ...
    trets.map! {|t| (t.is_a?(QDL::Type::AnnotatedArgType) || t.is_a?(QDL::Type::BoundArgType)) ? t.type : t}
    return [env, QDL::Type::UnionType.new(*trets)]
  end
...
=begin
    ...
    puts "----------------------"
=end
    if (trecv == QDL::Globals.types[:array])
      trecv = QDL::Type::GenericType.new(QDL::Globals.types[:array], QDL::Globals.types[:bot])
    elsif (trecv == QDL::Globals.types[:hash])
      trecv = QDL::Type::GenericType.new(QDL::Globals.types[:hash], QDL::Globals.types[:bot], QDL::Globals.types[:bot])
    elsif trecv.is_a?(QDL::Type::AnnotatedArgType) || trecv.is_a?(QDL::Type::DependentArgType)
      trecv = trecv.type
    end
    ...
    when QDL::Type::SingletonType
      if trecv.val.is_a? Class or trecv.val.is_a? Module
        if (meth == :new) && trecv.val.methods.include?(:new) && trecv.val.method(:new).owner == Class then ## last condition makes sure :new isn't overriden
          ...
          self_inst = QDL::Type::NominalType.new(trecv.val)
          ...
          meth_lookup = meth
          trecv_lookup = QDL::Util.add_singleton_marker(trecv.val.to_s)
          self_inst = trecv
        end
        ts = lookup(scope, trecv_lookup, meth_lookup, e)
        ts = [QDL::Type::MethodType.new([], nil, QDL::Type::NominalType.new(trecv.val))] if init && (ts.nil?) # there's always a nullary new if initialize is undefined
        ...
        if env[:self].is_a?(QDL::Type::NominalType)
          klass = env[:self].klass
        #elsif env[:self].is_a?(
        elsif env[:self].is_a?(QDL::Type::SingletonType) # SingletonType(class)
          klass = env[:self].val
        else
    ...
    when QDL::Type::AstNode
      meth_lookup = meth
      trecv_lookup = QDL::Util.add_singleton_marker(trecv.val.to_s)
      self_inst = trecv
      ts = lookup(scope, trecv_lookup, meth_lookup, e)
      ts = [QDL::Type::MethodType.new([], nil, QDL::Type::NominalType.new(trecv.val))] if (meth == :new) && (ts.nil?) # there's always a nullary new if initialize is undefined
      error :no_singleton_method_type, [trecv.val, meth], e unless ts
      inst = {self: self_inst}
      self_klass = trecv.val
    ...
    when QDL::Type::NominalType
      ...
      unless ts
        ...
        if klass.class == Module
          ...
          # TODO: Handle :extend
          nts = QDL::Globals.module_mixees[klass].map { |k, kind| if kind == :include then QDL::Type::NominalType.new(k) end }
          return [env, [QDL::Globals.types[:bot]]] if nts.empty? # if module not mixed in, this call can't happen; so %bot
          ut = QDL::Type::UnionType.new(*nts)
      ...
      inst = {self: trecv}
      self_klass = QDL::Util.to_class(trecv.name)
    when QDL::Type::GenericType
      ...
      if ts.nil? && (trecv.base.name == "ActiveRecord_Relation") && defined? DBType
        ...
        ts = ts.map { |t| QDL::Type::MethodType.new(t.args, t.block, trecv) }
      end
      error :no_instance_method_type, [trecv.base.name, meth], e unless ts
      inst = trecv.to_inst.merge(self: trecv)
      self_klass = QDL::Util.to_class(trecv.base.name)
    when QDL::Type::TupleType
      if QDL::Config.instance.use_comp_types
        ts = lookup(scope, "Array", meth, e)
        error :no_instance_method_type, ["Array", meth], e unless ts
        #inst = trecv.to_inst.merge(self: trecv)
        t_bind = trecv.promote.params[0].to_s == "t" ? QDL::Globals.types[:bot] : trecv.promote.params[0]
    ...
    when QDL::Type::FiniteHashType
      if QDL::Config.instance.use_comp_types
        ...
        #inst = trecv.to_inst.merge(self: trecv)
        k_bind = trecv.promote.params[0].to_s == "k" ? QDL::Globals.types[:bot] : trecv.promote.params[0]
        v_bind = trecv.promote.params[1].to_s == "v" ? QDL::Globals.types[:bot] : trecv.promote.params[1]
    ...
    when QDL::Type::VarType
      ...
      tactuals = tactuals.map { |t|
        if t.is_a?(QDL::Type::PreciseStringType)
          QDL::Globals.types[:string]
        elsif t.is_a?(QDL::Type::SingletonType) && !(t.val.class == Symbol) ## Symbol singletons come in handy for reconstructing finite hash types
          QDL::Type::NominalType.new(t.val.class)
        else
          t
        end }

      if meth == :to_s
        ret_type = QDL::Globals.types[:string]
      elsif meth == :to_i
        ret_type = QDL::Globals.types[:integer]
      else
        if @var_cache.has_key?(e.object_id) ## cache is based on syntactic location of method call
          ret_type = @var_cache[e.object_id]
        else
          ret_type = QDL::Type::VarType.new(cls: trecv, meth: meth, category: :ret, name: "ret")
          @var_cache[e.object_id] = ret_type
        end
      end

      if block
        if block[0].is_a?(QDL::Type::MethodType) || block[0].is_a?(QDL::Type::VarType)
          meth_type = QDL::Type::MethodType.new(tactuals, block[0], ret_type)
        else
          blk_args = block[0].children.map {|a| a.children[0]}
          blk_arg_vartypes = blk_args.map { |a|
            QDL::Type::VarType.new(cls: trecv, meth: meth, category: :block_arg, name: a.to_s ) }#block[0].children[0].to_s) }
          blk_ret_vartype = QDL::Type::VarType.new(cls: trecv, meth: meth, category: :block_ret, name: "block_ret")
          block_type = QDL::Type::MethodType.new(blk_arg_vartypes, nil, blk_ret_vartype)

          meth_type = QDL::Type::MethodType.new(tactuals, block_type, ret_type)
        ...
        meth_type = QDL::Type::MethodType.new(tactuals, nil, ret_type)
      end

      QDL::Type::Type.leq(trecv, QDL::Type::StructuralType.new({ meth => meth_type }), ast: e)
      #tmeth_inter = [meth_type]

      #self_klass = nil
    #error :recv_var_type, [trecv], e
      return [env, [ret_type]]
    when QDL::Type::MethodType
      if meth == :call
        # Special case - invokes the Proc
        ts = [trecv]
      else
        # treat as Proc
        return tc_send_one_recv(scope, env, QDL::Globals.types[:proc], meth, tactuals, block, e, op_asgn, union)
      end
    when QDL::Type::DynamicType
      return [env, [trecv]]
    else
    ...
    QDL::Type.expand_product(tactuals).each { |tactuals_expanded|
      ...
      ts.each_with_index { |tmeth, ind| # MethodType
        ...
        choice_num += 1
        current_ret = nil
        if tmeth.is_a? QDL::Type::DynamicType
          trets_tmp << (current_ret = QDL::Type::DynamicType.new)
        elsif ((tmeth.block && block) || (tmeth.block.nil? && block.nil?) || tmeth.block.is_a?(QDL::Type::VarType))
          if trecv.is_a?(QDL::Type::FiniteHashType) && trecv.the_hash
            trecv = trecv.canonical
            inst = trecv.to_inst.merge(self: trecv)
          end
          block_types = (if tmeth.block.is_a?(QDL::Type::MethodType) then tmeth.block.args + [tmeth.block.ret] else [] end)
          unless (tmeth.args+[tmeth.ret]+block_types).all? { |t| !t.instance_of?(QDL::Type::ComputedType) }
            tmeth_old = tmeth
          ...
          if tmeth_inst
            ...
            if init#trecv.is_a?(QDL::Type::SingletonType) && meth == :new
              init_typ = QDL::Type::NominalType.new(trecv.val)
              if (tmeth.ret.instance_of?(QDL::Type::GenericType))
                error :bad_initialize_type, [], e unless (tmeth.ret.base == init_typ)
              elsif (tmeth.ret.instance_of?(QDL::Type::AnnotatedArgType) || tmeth.ret.instance_of?(QDL::Type::DependentArgType) || tmeth.ret.instance_of?(QDL::Type::BoundArgType))
                error :bad_initialize_type, [], e unless (tmeth.ret.type == init_typ)
              else
                error :bad_initialize_type, [], e unless (tmeth.ret == init_typ) unless ((trecv.val == Hash) && tmeth.ret.is_a?(QDL::Type::FiniteHashType)) || ((trecv.val == Array) && tmeth.ret.is_a?(QDL::Type::TupleType))
              end
            #trets_tmp << init_typ unless block_mismatch
              ...
              got_match = true
              if comp_type && QDL::Config.instance.check_comp_types && !union
                if (e.type == :op_asgn) && op_asgn
                  ...
                  QDL::Globals.comp_type_map[e.object_id.object_id] = [tmeth, tmeth_old, tmeth_res, self_klass, trecv_old, targs_old, (binds || {})]
                else
                  QDL::Globals.comp_type_map[e.object_id] = [tmeth, tmeth_old, tmeth_res, self_klass, trecv_old, targs_old, (binds || {})]
                end
              end
            end
        ...
        if got_match
          ...
          if ts.size > 1 && tactuals_expanded.any? { |t| t.is_a?(QDL::Type::VarType) }
            ...
            constraints_hash = current_dcs.inject({}) do |hash, constraint|
              hash[constraint[0]] = QDL::Type::UnionType.new(hash[constraint[0]], constraint[1]).canonical
              hash
            end
            ...
            ret_choice[choice_num] = QDL::Type::UnionType.new(current_ret, ret_choice[choice_num]).canonical
          end
        end
    ...
    if arg_choices.empty?
      ...
      arg_choices.each { |vartype, choice_hash|
        if choice_hash.values.uniq.size == 1
          ...
          t = QDL::Type::ChoiceType.new(choice_hash)
          new_dcs << [vartype, t]
          ct_args << t
        end
      }
      ...
      if ret_choice.values.uniq.size == 1
        ...
        new_ret = QDL::Type::ChoiceType.new(ret_choice)
...
RUBY
      msg.chomp! # remove trailing newline
      name = if trecv.is_a?(QDL::Type::SingletonType) && trecv.val.is_a?(Class) && (meth == :new) then
        trecv.val.to_s
      elsif trecv.is_a? QDL::Type::SingletonType
        trecv.val.class.to_s
      elsif [QDL::Type::NominalType, QDL::Type::GenericType, QDL::Type::FiniteHashType, QDL::Type::TupleType, QDL::Type::AstNode, QDL::Type::PreciseStringType].any? { |t| trecv.is_a? t }
        trecv.to_s
      elsif trecv.is_a?(QDL::Type::MethodType)
        'Proc'
      else
  ...
  def self.apply_deferred_constraints(deferred_constraints, e)
    if deferred_constraints.size > 2 && deferred_constraints.all? { |t1, t2| t1.equal?(deferred_constraints[0][0]) && t2.is_a?(QDL::Type::NominalType) && t2 <= QDL::Globals.types[:numeric] }
    ...
    ## then only apply the single constraint that t1 <= Numeric.
      QDL::Type::Type.leq(deferred_constraints[0][0], QDL::Globals.types[:numeric], nil, false, ast: e)
    else
      deferred_constraints.each { |t1, t2| QDL::Type::Type.leq(t1, t2, nil, false, ast: e) }
    end
  end
  ...
  def self.compute_types(tmeth, self_klass, trecv, tactuals, binds={})
    ...
    tmeth.args.each { |targ|
      case targ
      when QDL::Type::ComputedType
        new_args << targ.compute(bind)
      when QDL::Type::BoundArgType
        if targ.type.instance_of?(QDL::Type::ComputedType)
    ...
    QDL::Type::MethodType.new(new_args, new_block, new_ret)
  end
  ...
  def self.tc_send_class(trecv, e)
    ...
    when QDL::Type::SingletonType
      if trecv.val.is_a? Class
        [QDL::Type::SingletonType.new(Class)]
      elsif trecv.val.is_a? Module
        [QDL::Type::SingletonType.new(Module)]
      else
        [QDL::Type::SingletonType.new(trecv.val.class)]
      end
    when QDL::Type::NominalType
      [QDL::Type::SingletonType.new(trecv.klass)]
    when QDL::Type::GenericType
      [QDL::Type::SingletonType.new(trecv.base.klass)]
    when QDL::Type::TupleType
      [QDL::Type::SingletonType.new(Array)]
    when QDL::Type::FiniteHashType
      [QDL::Type::SingletonType.new(Hash)]
    when QDL::Type::PreciseStringType
      [QDL::Type::SingletonType.new(String)]
    when QDL::Type::VarType
      [QDL::Type::NominalType.new(Class)]
      #error :recv_var_type, [trecv], e
    when QDL::Type::MethodType
      [QDL::Type::SingletonType.new(Proc)]
    else
  ...
  def self.tc_arg_types(tmeth, tactuals, deferred_constraints=[])
    ...
    until states.empty?
      ...
      t = tformals[formal]
      if t.instance_of?(QDL::Type::AnnotatedArgType) || t.instance_of?(QDL::Type::BoundArgType)
        t = t.type
      end
      case t
      when QDL::Type::OptionalType
        t = t.type
        if actual == tactuals.size
          states << [formal+1, actual, inst, deferred_constraints] # skip over optinal formal
        elsif (not tactuals[actual].is_a?(QDL::Type::VarargType)) && QDL::Type::Type.leq(tactuals[actual], t, inst, false, deferred_constraints) #&& (not (tactuals[actual].is_a?(QDL::Type::VarargType)))
          states << [formal+1, actual+1, inst, deferred_constraints] # match
          #states << [formal+1, actual, inst, deferred_constraints] unless tactuals[actual].is_a?(QDL::Type::VarType) && tformals[formal+1].is_a?(QDL::Type::FiniteHashType)# skip
        ...
        elsif tactuals[actual].is_a?(QDL::Type::VarargType) && QDL::Type::Type.leq(QDL::Type::GenericType.new(QDL::Globals.types[:array], tactuals[actual].type), t, inst, false, deferred_constraints)
          states << [formal+1, actual+1, inst, deferred_constraints] # match
        else
          states << [formal+1, actual, inst, deferred_constraints] # types don't match; must skip this formal
        end
      when QDL::Type::VarargType
        if actual == tactuals.size
          states << [formal+1, actual, inst, deferred_constraints] # skip to allow empty vararg at end
        elsif (not (tactuals[actual].is_a?(QDL::Type::VarargType))) && QDL::Type::Type.leq(tactuals[actual], t.type, inst, false, deferred_constraints)
        ...
        elsif tactuals[actual].is_a?(QDL::Type::VarargType) && QDL::Type::Type.leq(tactuals[actual].type, t.type, inst, false, deferred_constraints) #&& QDL::Type::Type.leq(t.type, tactuals[actual].type, inst, true, deferred_constraints)
        ...
        if actual == tactuals.size
          next unless t.instance_of? QDL::Type::FiniteHashType
          if @@empty_hash_type <= t
            states << [formal+1, actual, inst, deferred_constraints]
          end
        elsif (not (tactuals[actual].is_a?(QDL::Type::VarargType))) && QDL::Type::Type.leq(tactuals[actual], t, inst, false, deferred_constraints)
          states << [formal+1, actual+1, inst, deferred_constraints] # match!
  ...
  def self.tc_bind_arg_types(tmeth, tactuals)
    ...
    until states.empty?
      ...
      next if formal >= tformals.size # Too many actuals to match
      t = tformals[formal]
      if t.instance_of? QDL::Type::AnnotatedArgType
        t = t.type
      end
      case t
      when QDL::Type::OptionalType
        ...
        elsif (not (tactuals[actual].is_a?(QDL::Type::VarargType))) && QDL::Type::Type.leq(tactuals[actual], t, inst, false, [])
      ...
      when QDL::Type::VarargType
        ...
        elsif (not (tactuals[actual].is_a?(QDL::Type::VarargType))) && QDL::Type::Type.leq(tactuals[actual], t.type, inst, false, [])
        ...
        elsif tactuals[actual].is_a?(QDL::Type::VarargType) && QDL::Type::Type.leq(tactuals[actual].type, t.type, inst, false, []) #&&
                                                               #QDL::Type::Type.leq(t.type, tactuals[actual].type, inst, true, [])
      ...
      #states << [formal+1, actual+1, inst, binds]
        if actual == tactuals.size
          states << [formal+1, actual, inst, binds] # skip to allow empty vararg at end
        elsif (not (tactuals[actual].is_a?(QDL::Type::VarargType)))
          states << [formal, actual+1, inst, binds] # match, more varargs coming
          states << [formal+1, actual+1, inst, binds] # match, no more varargs
          states << [formal+1, actual, inst, binds] # skip over even though matches
        elsif tactuals[actual].is_a?(QDL::Type::VarargType)
          states << [formal+1, actual+1, inst, binds] # match, no more varargs; no other choices!
        else
          states << [formal+1, actual, inst, binds] # doesn't match, must skip
        end
        ...
        if actual == tactuals.size
          ...
          states << [formal+1, actual+1, inst, binds] if (t.is_a?(QDL::Type::ComputedType) || QDL::Type::Type.leq(tactuals[actual], t, inst, false, []))# match!
          # no else case; if there is no match, this is a dead end
        end
      end
    end
  ...
  def self.tc_block(scope, env, tblock, block, inst)
    ...
    raise RuntimeError, "block with block arg?" unless tblock.is_a?(QDL::Type::VarType) || tblock.block.nil?
    tblock = tblock.instantiate(inst)
    if block[0].is_a?(QDL::Type::MethodType) || block[0].is_a?(QDL::Type::VarType)
      error :bad_block_arg_type, [block[0], tblock], block[1], block: true unless QDL::Type::Type.leq(block[0], tblock, inst, false, ast: block[1])#block[0] <= tblock
      ret_env = env
    elsif block[0].is_a?(QDL::Type::NominalType) && block[0].name == 'Proc'
      error :proc_block_arg_type, [tblock], block[1], block: true
    elsif tblock.is_a?(QDL::Type::VarType)
      args, body = block
      arg_names = args.children.map { |a| a.children[0] }
      args_hash = {}
      arg_vartypes = arg_names.map { |a|
        v = QDL::Type::VarType.new(cls: inst[:self], meth: "block", category: :block_arg, name: a.to_s )
        args_hash[a] = v
        v
      }
      body_env, body_type = if body.nil? then [nil, QDL::Globals.types[:nil]] else tc(scope, env.merge(Env.new(args_hash)), body) end
      block_type = QDL::Type::MethodType.new(arg_vartypes, nil, body_type)
      QDL::Type::Type.leq(block_type, tblock, inst, false, ast: body)
      ...
      scope_merge(scope, outer_env: env) { |bscope|
        ...
        body_env, body_type = if body.nil? then [nil, QDL::Globals.types[:nil]] else tc(bscope, env_with_targs, body) end
        error :bad_return_type, [body_type, tblock.ret], body, block: true unless body.nil? || QDL::Type::Type.leq(body_type, tblock.ret, inst, false, ast: body)
        ret_env = env.merge_block_env(body_env, arg_names)
  ...
  def self.lookup(scope, klass, name, e, make_unknown: QDL::Config.instance.use_unknown_types)
    ...
    t = QDL::Globals.info.get_with_aliases(klass, name, :type)
    return t if t # simplest case, no need to walk inheritance hierarchy
    return [make_unknown_method_type(klass, name)] if QDL::Globals.to_infer.values.any? { |set| set.include?([klass, name]) }
    klass_name = QDL::Util.has_singleton_marker(klass) ? QDL::Util.remove_singleton_marker(klass) : klass
    return [make_unknown_method_type(klass, name)] if make_unknown && (name == :initialize) && (QDL::Util.to_class(klass_name).instance_method(:initialize).owner == QDL::Util.to_class(klass_name)) # don't want to walk up hierarchy in initialize case where it is specifically defined for this class
    the_klass = QDL::Util.to_class(klass)
    is_singleton = QDL::Util.has_singleton_marker(klass)
    included = QDL::Util.to_class(klass.gsub("[s]", "")).included_modules
    ...
    ancestors.each { |ancestor|
      ...
      tancestor = QDL::Globals.info.get_with_aliases(anc_lookup, name, :type)
      return tancestor if tancestor
      return [make_unknown_method_type(anc_lookup, name)] if QDL::Globals.to_infer.values.any? { |set| set.include?([anc_lookup, name]) }
      # special caes: Kernel's singleton methods are *also* added when included?!
      if ancestor == Kernel
        tancestor = QDL::Globals.info.get_with_aliases(QDL::Util.add_singleton_marker('Kernel'), name, :type)
        return tancestor if tancestor
      end
...
=end
    ...
    if QDL::Config.instance.assume_dyn_type
      ...
      if method
        ...
        args = arity.times.map { QDL::Globals.types[:dyn] }
        args << QDL::Type::VarargType.new(QDL::Globals.types[:dyn]) if has_varargs
      else
        args = [QDL::Type::VarargType.new(QDL::Globals.types[:dyn])]
      end

      ret = QDL::Globals.types[:dyn]
      ret = QDL::Type::NominalType.new(the_klass) if name == :initialize

      return [QDL::Type::MethodType.new(args, nil, ret)]
    else
      #return nil
      ## Trying new approach: create unknown method type for any methods without types.
      if make_unknown
        return [make_unknown_method_type(klass, name)] if (QDL::Util.to_class(klass).instance_methods + QDL::Util.to_class(klass).private_instance_methods).include?(name)
      else
        return nil
      end
    end
  ...
  def self.make_unknown_method_type(klass, meth)
    raise "Tried to make unknown method type for class #{klass} method #{meth}, but no such method was found." unless (QDL::Util.to_class(klass).instance_methods + QDL::Util.to_class(klass).private_instance_methods).include?(meth)

    klass_obj = QDL::Util.to_class(klass)
    ...
    params.each { |param|
      ...
      case param[0]
      when :req
        arg_types << QDL::Type::VarType.new(cls: klass, meth: meth, category: :arg, name: var_name)
      when :opt
        arg_types << QDL::Type::OptionalType.new(QDL::Type::VarType.new(cls: klass, meth: meth, category: :arg, name: var_name))
      when :rest
        arg_types << QDL::Type::VarargType.new(QDL::Type::VarType.new(cls: klass, meth: meth, category: :arg, name: var_name))
      when :key
        keyword_args[var_name] = QDL::Type::OptionalType.new(QDL::Type::VarType.new(cls: klass, meth: meth, category: :arg, name: var_name))
      when :keyreq
        keyword_args[var_name] = QDL::Type::VarType.new(cls: klass, meth: meth, category: :arg, name: var_name)
      when :block
    ...
    keyword_args = keyword_args.empty? ? [] : [QDL::Type::FiniteHashType.new(keyword_args, nil)]
    arg_types = arg_types + keyword_args
    if meth == :initialize
=begin
      if QDL::Util.has_singleton_marker(klass)
        # to_class gets the class object itself, so remove singleton marker to get class rather than singleton class
        ret_vartype = QDL::Type::SingletonType.new(QDL::Util.to_class(QDL::Util.remove_singleton_marker(klass)))
      else
        ret_vartype = QDL::Type::NominalType.new(klass)
      end
=end
      ret_vartype = QDL::Globals.parser.scan_str "#T self"
    #ret_vartype = QDL::Type::VarType.new(:self) ## TODO: is this right? Or should it include klass/meth info?
    else
      ret_vartype = QDL::Type::VarType.new(cls: klass, meth: meth, category: :ret, name: "ret")
    end

    block_type = QDL::Type::VarType.new(cls: klass, meth: meth, category: :block, name: "block")

    meth_type = QDL::Type::MethodType.new(arg_types, block_type, ret_vartype)
    QDL::Globals.info.add(klass, meth, :type, meth_type)
    return meth_type
  end

  def self.make_unknown_var_type(klass, name, kind_text)
    var_type = QDL::Type::VarType.new(cls: klass, name: name, category: kind_text)
    QDL::Globals.info.set(klass, name, :type, var_type)
    QDL::Globals.constrained_types << [klass, name]
    return var_type
  end
  ...
  def self.filter_comp_types(ts, use_dep_types)
    ...
    ts.each { |typ|
      case typ
      when QDL::Type::MethodType
        block_types = (if typ.block.is_a?(QDL::Type::MethodType) then typ.block.args + [typ.block.ret] else [] end)
        typs = typ.args + block_types + [typ.ret]
        if typs.any? { |t| t.is_a?(QDL::Type::ComputedType) || (t.is_a?(QDL::Type::BoundArgType) && t.type.is_a?(QDL::Type::ComputedType))  }
          dep_ts << typ
        else
          non_dep_ts << typ
        end
  ...
  def self.get_singleton_name(name)
    ...
    new_name = QDL::Util.add_singleton_marker($1)
    new_name
  end
  ...
  def self.find_constant(env, scope, e)
    ...
    if (QDL::Util.has_singleton_marker(scope[:klass]))
      klass = QDL::Util.to_class(QDL::Util.remove_singleton_marker(scope[:klass]))
      mod_inst = false
    else
      klass = QDL::Util.to_class(scope[:klass])
    ...
    if QDL::Wrap.wrapped?(scope[:klass], scope[:meth])
      meth_name = QDL::Wrap.wrapped_name(scope[:klass], scope[:meth])
    else
...
# Use parser's Diagnostic to output QDL typechecker error messages
class Diagnostic < Parser::Diagnostic

  def message
    QDL_MESSAGES[@reason] % @arguments
  end
...
class Object
  ...
  ## [+ __qdl_meth +] is a Symbol naming the method being replaced.
  ...
  def __qdl_dyn_type_check(__qdl_meth, node_id, *args, &block)
    tmeth, tmeth_old, tmeth_res, self_klass, trecv_old, targs_old, binds = QDL::Globals.comp_type_map[node_id]
    raise RuntimeError, "Could not find cached type-level computation results for method #{__qdl_meth}." unless tmeth
    if QDL::Config.instance.rerun_comp_types
      tmeth_new = QDL::Typecheck.compute_types(tmeth_old, self_klass, trecv_old, targs_old, binds)
      unless tmeth_new == tmeth_res
        raise QDL::Type::TypeError, "Type-level computation evaluated to different result from type checking time for class #{self_klass} method #{__qdl_meth}.\n Got #{tmeth_res} the first time, but #{tmeth_new} the second time."
      end
    end
    bind = binding
    inst = nil
    ...
    matches, args, _, bind = QDL::Type::MethodType.check_arg_types("#{__qdl_meth}", self, bind, [tmeth], inst, *args, &block)

    ret = self.send(__qdl_meth, *args, &block)

    if matches
      ret = QDL::Type::MethodType.check_ret_types(self, "#{__qdl_meth}", [tmeth], inst, matches, ret, bind, *args, &block) unless __qdl_meth == :initialize
    end
...
class WrapCall < Parser::TreeRewriter
  ...
  def on_send(node)
    ...
    unless node.children[1] == :__qdl_dyn_type_check ## I don't believe this check is necessary, but at one point I had this issue so I'm leaving it in
      if QDL::Globals.comp_type_map[node.object_id] ## Only do this if a call is associated with a type in the map. Otherwise, it may be a call to a non-dependently typed method.
        align_replace(node.location.expression, @offset, "#{rec_code}__qdl_dyn_type_check(:#{node.children[1]}, #{node.object_id} #{args_code})")
      end
    end
  end
  ...
  def on_op_asgn(node)
    if node.children[0].type == :send
      ...
      if QDL::Globals.comp_type_map[node.object_id]
        op_code = "#{rec_meth_code}.__qdl_dyn_type_check(:#{op_meth}, #{node.object_id}, #{rhs_code})"
      else
        op_code = "#{rec_meth_code}.send(:#{op_meth}, #{rhs_code})"
      end

      if QDL::Globals.comp_type_map[node.object_id.object_id]
        align_replace(node.location.expression, @offset, "#{rec_code}__qdl_dyn_type_check(:#{mutation_meth}, #{node.object_id.object_id}, #{elargs_code}, #{op_code})")
      end
    else
      ...
      rhs_code = WrapCall.rewrite(rhs_ast)
      if QDL::Globals.comp_type_map[node.object_id]
        align_replace(node.location.expression, @offset, "#{lhs} = #{lhs}.__qdl_dyn_type_check(:#{meth}, #{node.object_id}, #{rhs_code})")
      end
    end
  end
  ...
  def on_or_asgn(node)
    if node.children[0].type == :send
      ...
      if QDL::Globals.comp_type_map[node.object_id]
        align_replace(node.location.expression, @offset, "#{rec_code}__qdl_dyn_type_check(:#{mutation_meth}, #{node.object_id}, #{elargs_code}, #{rec_meth_code} || #{rhs_code})")
      end
    else
      lhs = node.location.name.source
      rhs_ast = node.children[1]
