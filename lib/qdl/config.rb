...
class QDL::Config
  ...
  def initialize
    QDL::Config.reset(self)
  end

  def self.reset(c=QDL::Config.instance)
  ...
  def add_nowrap(*klasses)
    klasses.each { |klass|
      ...
      @nowrap.add QDL::Util.add_singleton_marker(klass.to_s).to_sym
    }
  end
  ...
  def remove_nowrap(*klasses)
    klasses.each { |klass|
      ...
      @nowrap.delete QDL::Util.add_singleton_marker(klass.to_s).to_sym
    }
  end

  # To use, copy these 3 lines to the test file of a gem
=begin
require_relative '../qdl3/qdl/lib/qdl.rb'
require_relative '../qdl3/qdl/lib/qdl_types.rb'
...
=end
  def profile_stats(outname="/#{__FILE__[0...-3]}",outdir="")
    ...
    at_exit do
      ...
      QDL::Globals.contract_switch.off {
        puts "START."
        ...
        p "Scanning QDL Contract Log"
        QDL::Globals.wrapped_calls.each{ |mname,ct|
        ...
        filtered = {}
        totals.each{ |k,v|
          if (not (k=~/(qdl)|(QDL)/)) and (v[0].nil? or v[0]==-1) then filtered[k]=v end
        }

        puts "Writing Output"
        require 'json'
        fpath = "#{outdir}/#{outname}_qdlstat.json".gsub('//','')
  ...
  def do_report
    ...
    typechecked = []
    missing = []
    QDL::Globals.info.info.each_pair { |klass, meths|
      meths.each { |meth, kinds|
        if kinds[:typecheck]
          ...
          raise RuntimeError, "#{QDL::Util.pp_klass_method(klass, meth)} typechecked but not annotated to do so?!"
        end
      }
    }
    unless typechecked.empty?
      puts "TYPECHECKED METHODS:"
      typechecked.each { |klass, meth| puts QDL::Util.pp_klass_method(klass, meth) }
    end
    unless missing.empty?
      puts unless typechecked.empty?
      puts "METHODS ANNOTATED TO BE TYPECHECKED BUT NOT TYPECHECKED:"
      missing.each { |klass, meth| puts QDL::Util.pp_klass_method(klass, meth) }
    end
  end
  ...
  def do_get_meth_type(klass, meth, the_meth)
    ...
    otypes = QDL::Globals.info.get(klass, meth, :otype) if QDL::Globals.info.has?(klass, meth, :otype) # observed types
    return if otypes.nil?
    #otargs = []
    otargs = {}
    otret = QDL::Globals.types[:bot]
    otblock = false
    binding_meth_name = "QDL_#{klass}_#{(meth.hash + meth.to_s.hash).abs}".gsub("::", "__").gsub("[s]", "singleton_")
    otypes.each { |ot|
      binds_hash = QDL.send(binding_meth_name, *ot[:args])
      binds_hash.each { |param_name, typ|
        if param_types[param_name] == :rest
          ...
          new_typ = QDL::Type::UnionType.new(*typ).canonical
        else
          new_typ = typ.is_a?(QDL::Type::Type) ? typ : QDL::Wrap.val_to_type(typ) ## may not be a Type for default arguments
        end
        otargs[param_name] = otargs[param_name] ? QDL::Type::UnionType.new(otargs[param_name], new_typ).canonical : new_typ
      }
      otret = QDL::Type::UnionType.new(otret, ot[:ret]).canonical if defined? otret
      otblock = otblock || ot[:block]
    }
    require 'method_source'
    otargs.each { |param, typ| if param_types[param] == :rest then otargs[param] = QDL::Type::VarargType.new(typ) end }
    printed_args = ""
  ...
  def guess_meth(klass, meth, is_sing, the_meth)
    ...
    otypes.each { |ot|
      ot[:args].each_with_index { |t, i|
        otargs[i] = QDL::Globals.types[:bot] if otargs[i].nil?
        begin
          otargs[i] = QDL::Type::UnionType.new(otargs[i], t).canonical
        rescue NameError; end
      }
      otret = QDL::Type::UnionType.new(otret, QDL::Type::NominalType.new(ot[:ret])).canonical if defined? otret
      otblock = otblock || ot[:block]
    }
    otargs.each { |t|
      print ", " unless first
  ...
  def do_guess_types
    ...
    puts "------------------------------"
    puts "TYPE GUESSES"
    QDL::Config.instance.guess_types.each { |klass|
      puts
      puts "class #{klass}"
      the_klass = QDL::Util.to_class(klass)
      sklass = QDL::Util.add_singleton_marker(klass.to_s)
      the_klass.singleton_methods(false).each { |meth|
        next unless meth.to_s =~ /^__qdl_(.*)_old/
  ...
  def do_get_types
    ...
    QDL::Config.instance.get_types.each { |klass, meth|
      # the_klass = QDL::Util.to_class(klass)
      # sklass = QDL::Util.add_singleton_marker(klass.to_s)
      wrapped_name = QDL::Wrap.wrapped_name(klass, meth)
      begin
        the_meth = QDL::Util.to_class(klass).instance_method(wrapped_name)
...
private
  ...
  def install_at_exit
    ...
    at_exit do
      QDL::Config.instance.do_report
      QDL::Config.instance.do_guess_types
      QDL::Config.instance.do_get_types
    end
