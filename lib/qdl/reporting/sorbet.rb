...
module QDL::Reporting::Sorbet

  def to_sorbet_type(typ)
    QDL::Logging.debug :reporting, typ

    typ = typ.canonical

    case typ
    when QDL::Type::StructuralType
      QDL::Globals.types[:dyn]

    when QDL::Type::VarType
      return 'self' if typ.to_s == 'self'
      QDL::Globals.types[:dyn]

    when QDL::Type::BotType
      QDL::Globals.types[:dyn]

    when QDL::Type::UnionType
      if typ.is_a? QDL::Type::UnionType
        types = typ.types.map { |x| to_sorbet_type(x) }
        QDL::Type::UnionType.new(*types)
      else
        typ
      end

    when QDL::Type::IntersectionType
      if typ.is_a? QDL::Type::IntersectionType
        types = typ.types.map { |x| to_sorbet_type(x) }
        QDL::Type::IntersectionType.new(*types)
      else
        typ
      end
    ...
    when QDL::Type::FiniteHashType
      c = typ#.canonical
      types = c.elts.values.map { |v| v.is_a?(QDL::Type::OptionalType) ? to_sorbet_type(v.type) : to_sorbet_type(v) }

      return QDL::Globals.types[:dyn] if types.member? QDL::Globals.types[:dyn]

      base_type = QDL::Globals.types[:hash]
      key_type = QDL::Globals.types[:symbol]
      val_type = QDL::Type::UnionType.new(*types)

      QDL::Type::GenericType.new base_type, key_type, val_type
  ...
  def to_sorbet_string(typ, header, in_hash: false)
    QDL::Logging.debug :reporting, "Processing #{header}..."

    unless typ
      QDL::Logging.error :reporting, "Given nil instead of QDL Type!"
      # return 'T.untyped'
      raise "Given nil instead of QDL Type!"
    end
    ...
    when QDL::Type::UnionType
      type = typ.canonical
      if type.is_a? QDL::Type::UnionType
    ...
    when QDL::Type::IntersectionType
      type = typ.canonical
      if type.is_a? QDL::Type::IntersectionType
        types = type.types.map { |x| to_sorbet_string(x, header, in_hash: in_hash) }
        "T.all(#{types.join(', ')})"
      else
        to_sorbet_string(type, header, in_hash: in_hash)
    ...
    when QDL::Type::TupleType
      "T::Array[#{to_sorbet_string QDL::Type::UnionType.new(*typ.params), header, in_hash: in_hash}]"

    when QDL::Type::TopType
      "T.nilable(BasicObject)" 
    else
      #QDL::Logging.warning :reporting, "Unmatched class #{typ.class}"
      'T.untyped'
  ...
  def gen_sorbet(generator)
    @methods.each do |method|
      ...
      header = QDL::Util.pp_klass_method(full_name, method.method_name)
      ...
      parameters = m.args.map do |typ|
        ...
        when QDL::Type::OptionalType
          ...
          QDL::Logging.log :reporting, :error, "Attempting to map #{typ} (a #{typ.class})"
        end
      end

      block_arg = Proc.new { to_sorbet_string(m.block.solution, header) } if m.block.solution

      unless m.ret.solution.is_a?(QDL::Type::SingletonType) &&
      ...
      QDL::Type::VarType.print_XXX!
      meth.add_comment "QDL Type: #{m.solution}"
    end

    @children.each_key do |key|
      child = @children[key]
      klass = QDL::Util.to_class(child.full_name)
