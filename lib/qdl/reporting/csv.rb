...
module QDL::Reporting::CSV
  ...
  def to_csv(path, open_file = nil)
    ...
    @methods.each do |method|
      ...
      QDL::Logging.debug :inference, "Rendering #{QDL::Util.pp_klass_method(class_str, method.method_name)}"

      if method.type.solution.is_a?(QDL::Type::MethodType)
        meth = method.type.solution
        inf_type = meth_to_s meth
      else
        QDL::Logging.warning :inference, "Got a non-method type in type solutions: #{method.type.class}"
