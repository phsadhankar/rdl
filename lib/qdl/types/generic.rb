  ...
  class GenericType < Type
    ...
    def member?(obj, *args)
      if base.name == "Table"
        ...
        raise QDL::Type::TypeError, "Expected Table type to be parameterized by finite hash, instead got #{@params}." unless @params[0].is_a?(QDL::Type::FiniteHashType)
        if @params[0].elts[:__all_joined].is_a?(QDL::Type::UnionType) && obj.joined_dataset?
        ...
        elsif !@params[0].elts[:__all_joined].is_a?(QDL::Type::TupleType) && !obj.joined_dataset?
          return true
        else
          return false
        end
      ...
      raise "No type parameters defined for #{base.name}" unless QDL::Globals.type_params[base.name]
#      formals = QDL::Globals.type_params[base.name][0]
    ...
    def to_inst
      return QDL::Globals.type_params[base.name][0].zip(@params).to_h
    end
