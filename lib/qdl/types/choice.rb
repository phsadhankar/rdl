  ...
  class ChoiceType < Type
    ...
    def initialize(choices, connecteds=[])
      ...
      if choices.values.any? { |t| t.is_a? ChoiceType }
        QDL::Logging.log :inference, :critical, "WARNING: Generating a ChoiceType with nested ChoiceTypes."
      end
    end
    ...
    def copy
      ...
      new_connecteds = @connecteds.map { |c| QDL::Type::ChoiceType.new(c.choices.transform_values { |t| t.copy }) }
