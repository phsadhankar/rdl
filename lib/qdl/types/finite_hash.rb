...
module QDL::Type
  ...
  class FiniteHashType < Type
    ...
    def promote(key=nil, value=nil)
      ...
      #domain_type = (@elts.empty? && !key) ? QDL::Type::VarType.new({ name: :k, cls: Hash, to_infer: true }) : UnionType.new(*(@elts.keys.map { |k| NominalType.new(k.class) }), key)
      if @elts.empty? && !key
        domain_type = QDL::Type::VarType.new(:k)
      else
        domain_type = UnionType.new(*@elts.keys.map { |k| if k.is_a?(Type) then k else NominalType.new(k.class) end }, key)
      end
      #range_type = (@elts.empty? && !value) ? QDL::Type::VarType.new({ name: :v, cls: Hash, to_infer: true }) : UnionType.new(*@elts.values, value)
      if @elts.empty? && !value
        range_type = QDL::Type::VarType.new(:v)
      else
        range_type = UnionType.new(*@elts.values.map { |v| if v.is_a?(OptionalType) then v.type else v end }, value)
      end

      if @rest
        domain_type = UnionType.new(domain_type, QDL::Globals.types[:symbol])
        range_type = UnionType.new(range_type, @rest)
      end
      if QDL::Config.instance.promote_widen
        case range_type
      ...
      x = GenericType.new(QDL::Globals.types[:hash], domain_type.canonical, range_type.canonical)
      return x#GenericType.new(QDL::Globals.types[:hash], domain_type.canonical, range_type.canonical)
    end
