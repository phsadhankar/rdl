module QDL::Type
  class UnionType < Type
    ...
    def self.new(*types)
      return QDL::Globals.types[:bot] if types.size == 0
      ts = []
    ...
    def drop_vars
      return QDL::Globals.types[:bot] if @types.all? { |t| t.is_a? VarType } ## when all are VarTypes, we have nothing concrete to reduce to, so don't want to drop vars
      ...
      QDL::Type::UnionType.new(*new_types).canonical
    end
