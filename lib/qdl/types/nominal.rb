module QDL::Type
  class NominalType < Type
    ...
    def self.new(name)
      name = name.to_s
      name = "Integer" if QDL::Config.instance.number_mode && ["Float", "Rational", "Complex", "BigDecimal", "BigDecimal::ROUND_MODE", "Numeric"].include?(name)
      t = @@cache[name]
      return t if t
      t = self.__new__ name
    ...
    def to_s
      if @name.start_with? '#<Class:'
        ...
        return QDL::Util.add_singleton_marker(n[8..-2])
      elsif QDL::Config.instance.number_mode && (@name == "Integer")
        return "Number"
      else
        return @name
      end
    ...
    def klass
      @klass = QDL::Util.to_class(name) unless defined? @klass
      return @klass
    end
