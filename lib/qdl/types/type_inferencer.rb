  ...
  class TypeInferencer
    def self.infer_type(it)
      ...
      if current_types.size == 1
        ...
        QDL::Globals.types[:nil]
      else
        QDL::Type::UnionType.new(*self.unify_param_types(current_types))
      end
    end
    ...
    def self.extract_types(param_type)
      param_type.instance_of?(QDL::Type::UnionType) ? param_type.types.to_a : [param_type]
    end
    ...
    def self.unify_param_types(type_set)
      ...
      type_set.each {|member_type|
        if member_type.instance_of? QDL::Type::GenericType
      ...
      parameterized_classes.each {|nominal, ts|
        nt = ts.map {|unioned_type_parameter|
          QDL::Type::UnionType.new(*unify_param_types(unioned_type_parameter))
        }

        non_param_classes << QDL::Type::GenericType.new(nominal, *nt)
      }
