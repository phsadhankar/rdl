module QDL::Type
  ...
  class Type
    ...
    def name
      QDL::Logging.log :typecheck, :error, "Attempted to access name field for #{self.class}"
      '_NAME_ERROR'
    end
    ...
    def solution=(soln)
      @solution = soln
      #QDL::Logging.log :typecheck, :warning, "Solution written to #{self.class}"
    end
    ...
    def to_contract
      ...
      c = QDL::Contract::FlatContract.new(to_s) { |obj|
        raise TypeError, "Expecting #{to_s}, got object of class #{QDL::Util.qdl_type_or_class(obj)}" unless slf.member?(obj)
        true
      }
    ...
    def array_type?
      is_a?(TupleType) || (is_a?(GenericType) && ((base == QDL::Globals.types[:array]) || ((defined? ActiveRecord_Relation) && (base.klass == ActiveRecord_Relation)))) || (is_a?(NominalType) && (self == QDL::Globals.types[:array])) || (is_a?(UnionType) && types.all? { |t| t.array_type? })
    end

    def hash_type?
      is_a?(FiniteHashType) || (is_a?(GenericType) && (base == QDL::Globals.types[:hash])) || (self == QDL::Globals.types[:hash]) || (is_a?(UnionType) && types.all? { |t| t.hash_type? })
    end
    ...
    def self.leq(left, right, inst=nil, ileft=true, deferred_constraints=nil, no_constraint: false, ast: nil, propagate: false, new_cons: {}, removed_choices: {})
      ...
      if left.is_a?(VarType) && !left.to_infer && right.is_a?(VarType) && !right.to_infer
        ...
        QDL::Logging.log :typecheck, :trace, "#{right}.is_a VarType"
        QDL::Logging.log :typecheck, :trace, "\t#{left} <= #{right}"
        if deferred_constraints.nil?
          QDL::Logging.log :typecheck, :trace, 'no deferred_constraints'
          right.add_lbound(left, ast, new_cons, propagate: propagate) unless (right.lbounds.any? { |t, loc| t == left || t.hash == left.hash } || right.equal?(left))
        else
          QDL::Logging.log :typecheck, :trace, 'deferred_constraints:'
          deferred_constraints << [left, right]
          deferred_constraints.each { |k, v| if v.is_a?(Array) then v.each { |v| QDL::Logging.log(:typecheck, :trace, "#{k} <= #{v[1] || v}") } else QDL::Logging.log(:typecheck, :trace, "#{k} <= #{v}") end }
        end
        return true
      end
      ...
      if left.is_a?(ChoiceType) && right.is_a?(ChoiceType)
        ...
        main_ct.choices.each { |num, t|
          ...
          if check
            new_dcs.each { |t1, t2|
              ub_var_choices[t1][num] = QDL::Type::UnionType.new(ub_var_choices[t1][num], t2).canonical if t1.is_a?(VarType)
              lb_var_choices[t2][num] = QDL::Type::UnionType.new(lb_var_choices[t2][num], t1).canonical if t2.is_a?(VarType)
            }
          else
            to_remove << num
          end
          ...
          if !((lb_var_choices.empty?) && (ub_var_choices.empty?))
            ...
            ub_var_choices.each { |vartype, choice_hash|
              if choice_hash.values.uniq.size == 1
                QDL::Type::Type.leq(vartype, choice_hash.values[0], nil, false, deferred_constraints, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices)
              else
                t = QDL::Type::ChoiceType.new(choice_hash)
                QDL::Type::Type.leq(vartype, t, nil, false, deferred_constraints, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices)
                all_cts << t
              end
            }

            lb_var_choices.each { |vartype, choice_hash|
              QDL::Logging.log :typecheck, :trace, vartype.to_s
              if choice_hash.values.uniq.size == 1
                QDL::Type::Type.leq(choice_hash.values[0], vartype, nil, false, deferred_constraints, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices)
              else
                t = QDL::Type::ChoiceType.new(choice_hash)
                QDL::Type::Type.leq(t, vartype, nil, false, deferred_constraints, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices)
                all_cts << t
              end
            }
      ...
      if (left.is_a?(NominalType) || left.is_a?(TupleType) || left.is_a?(FiniteHashType) || left.is_a?(TopType) || left.is_a?(PreciseStringType) || (left.is_a?(SingletonType) && !left.val.nil?)) && right.is_a?(StructuralType)
        ...
        when TupleType
          ...
          t_bind = left.promote.params[0].to_s == "t" ? QDL::Globals.types[:bot] : left.promote.params[0]
          base_inst[:t] = t_bind
        when FiniteHashType
          lklass = Hash
          base_inst = { self: left }
          # hack
          k_bind = left.promote.params[0].to_s == "k" ? QDL::Globals.types[:bot] : left.promote.params[0]
          v_bind = left.promote.params[1].to_s == "v" ? QDL::Globals.types[:bot] : left.promote.params[1]
          base_inst[:k] = k_bind
          base_inst[:v] = v_bind
        ...
        when TopType
          lklass = Object
          base_inst = { self: QDL::Globals.types[:object] }
        when SingletonType
          base_inst = { self: left }
          ...
          if (left == QDL::Globals.types[:array]) || (left == QDL::Type::NominalType.new(Range))
            lklass = left.klass
            left = QDL::Type::GenericType.new(left, QDL::Globals.types[:bot])
            base_inst = { self: left, t: QDL::Globals.types[:bot] }
          elsif (left == QDL::Globals.types[:hash])
            left = QDL::Type::GenericType.new(QDL::Globals.types[:hash], QDL::Globals.types[:bot], QDL::Globals.types[:bot])
            lklass = Hash
            base_inst = { self: left, k: QDL::Globals.types[:bot], v: QDL::Globals.types[:bot] }
          else
            lklass = left.klass
            base_inst = { self: left }
          end
        ...
        right.methods.each_pair { |m, t|
          return false unless lklass.method_defined?(m) || QDL::Typecheck.lookup({}, klass_lookup, m, nil, make_unknown: false)#QDL::Globals.info.get(lklass, m, :type) ## Added the second condition because Rails lazily defines some methods.
          types = QDL::Typecheck.lookup({}, lklass.to_s, m, nil, make_unknown: false)#QDL::Globals.info.get(lklass, m, :type)
          if QDL::Config.instance.use_comp_types
            types = QDL::Typecheck.filter_comp_types(types, true)
          else
            types = QDL::Typecheck.filter_comp_types(types, false)
          end
          ...
          if types
            ...
            types.each { |tlm|
              choice_num += 1
              blk_typ = tlm.block.is_a?(QDL::Type::MethodType) ? tlm.block.args + [tlm.block.ret] : [tlm.block]
              if (tlm.args + blk_typ + [tlm.ret]).any? { |t| t.is_a? ComputedType }
                ...
                tlm = QDL::Typecheck.compute_types(tlm, lklass, left, t.args)
              end
              new_dcs = []
              if leq(tlm.instantiate(base_inst), t, nil, true, new_dcs, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices)
                ...
                if types.size > 1 && !new_dcs.empty? ## method has intersection type, and vartype constraints were created
                  new_dcs.each { |t1, t2|
                    ub_var_choices[t1][choice_num] = QDL::Type::UnionType.new(ub_var_choices[t1][choice_num], t2).canonical if t1.is_a?(VarType)
                    lb_var_choices[t2][choice_num] = QDL::Type::UnionType.new(lb_var_choices[t2][choice_num], t1).canonical if t2.is_a?(VarType)
                  }
                else
                  new_dcs.each { |t1, t2| QDL::Type::Type.leq(t1, t2, nil, ileft, deferred_constraints, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices) }
                end
              end
            }
            ...
            if !((lb_var_choices.empty?) && (ub_var_choices.empty?))
              ...
              ub_var_choices.each { |vartype, choice_hash|
                if choice_hash.values.uniq.size == 1
                  QDL::Type::Type.leq(vartype, choice_hash.values[0], nil, ileft, deferred_constraints, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices)
                else
                  t = QDL::Type::ChoiceType.new(choice_hash)
                  QDL::Type::Type.leq(vartype, t, nil, ileft, deferred_constraints, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices)
                  all_cts << t
                end
              }
              ...
              lb_var_choices.each { |vartype, choice_hash|
                if choice_hash.values.uniq.size == 1
                  QDL::Type::Type.leq(choice_hash.values[0], vartype, nil, ileft, deferred_constraints, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices)
                else
                  t = QDL::Type::ChoiceType.new(choice_hash)
                  QDL::Type::Type.leq(t, vartype, nil, ileft, deferred_constraints, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices)
                  all_cts << t
                end
              }
      ...
      if left.is_a?(GenericType) && right.is_a?(GenericType)
        formals, variance, _ = QDL::Globals.type_params[left.base.name]
      ...
      if left.is_a?(GenericType) && right.is_a?(StructuralType)
        ...
        right.methods.each_pair { |meth, t|
          if (klass.to_s == "ActiveRecord_Relation") && !klass.method_defined?(meth) && defined? DBType
            types = QDL::Typecheck.lookup({}, klass.to_s, meth, {}, make_unknown: false)
            if !types
              base_types = QDL::Typecheck.lookup({}, "[s]"+DBType.rec_to_nominal(left).name, meth, {}, make_unknown: false)
              return false unless base_types
              types = base_types.map { |t| QDL::Type::MethodType.new(t.args, t.block, left) }
            end
            return false unless types
          else
            return false unless klass.method_defined?(meth) || QDL::Typecheck.lookup({}, klass.to_s, meth, nil, make_unknown: false)#QDL::Globals.info.get(klass, meth, :type) ## Added the second condition because Rails lazily defines some methods.
            types = QDL::Typecheck.lookup({}, klass.to_s, meth, {}, make_unknown: false)#QDL::Globals.info.get(klass, meth, :type)
          end
          ...
          if types
            ...
            types.each { |tlm|
              choice_num += 1
              blk_typ = tlm.block.is_a?(QDL::Type::MethodType) ? tlm.block.args + [tlm.block.ret] : [tlm.block]
              tlm = QDL::Typecheck.compute_types(tlm, klass, left, t.args) if (tlm.args + blk_typ + [tlm.ret]).any? { |t| t.is_a? ComputedType }
              new_dcs = []
              if leq(tlm.instantiate(base_inst.merge({ self: left})), t, nil, true, new_dcs, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices)
                ...
                if types.size > 1 && !new_dcs.empty? ## method has intersection type, and vartype constraints were
                  ...
                  new_dcs.each { |t1, t2| QDL::Type::Type.leq(t1, t2, nil, false, deferred_constraints, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices) }
                end
              end
            }
            if !((lb_var_choices.empty?) && (ub_var_choices.empty?))
      ...
      if left.is_a?(MethodType) && right.is_a?(MethodType)
        ...
        if left.args.last.is_a?(VarargType)
          ...
          if right.args.size >= left.args.size
            ...
            if left.args.size == 1
              left = QDL::Type::MethodType.new(new_args, left.block, left.ret)
            else
              left = QDL::Type::MethodType.new(left.args[0..(left.args.size-2)]+new_args, left.block, left.ret)
            end
          elsif right.args.size == left.args.size
            left = QDL::Type::MethodType.new(left.args[0..left.args.size-2] + [left.args[-1].type], left.block, left.ret)
          else
            left = QDL::Type::MethodType.new(left.args[0..left.args.size-2], left.block, left.ret)
          end
        end

        last_arg = left.args.last
        if last_arg.is_a?(OptionalType) || (last_arg.is_a?(AnnotatedArgType) && last_arg.type.is_a?(OptionalType))
          left = QDL::Type::MethodType.new(
            left.args.map { |t|
          ...
          ## of a method without the optional type. So drop it and then check subtyping.
            left = QDL::Type::MethodType.new(left.args.slice(0, left.args.size-1), left.block, left.ret)
          end
        end
      ...
      if left.is_a?(TupleType) && right.is_a?(GenericType) && right.base == QDL::Globals.types[:array]
      ...
      if left.is_a?(FiniteHashType) && right.is_a?(GenericType) && right.base == QDL::Globals.types[:hash]
        # TODO !ileft and right carries a free variable
        return false unless left.promote!
        return leq(left, right, inst, ileft, deferred_constraints, no_constraint: no_constraint, ast: ast, propagate: propagate, new_cons: new_cons, removed_choices: removed_choices) # recheck for promoted type
      end
      ...
      if left.is_a?(PreciseStringType)
        if right.is_a?(PreciseStringType)
          ...
          right.lbounds << left unless no_constraint
          return true
        elsif right == QDL::Globals.types[:string]
          return false unless left.promote!
          return true
        elsif right.is_a?(NominalType) && String.ancestors.include?(QDL::Util.to_class(right.name))
