  ...
  class VarType < Type
    ...
    def initialize(name_or_hash)
      if name_or_hash.is_a?(Symbol) || name_or_hash.is_a?(String)
        ...
        raise "Unexpected argument #{name_or_hash} to QDL::Type::VarType.new."
      end
    end
    ...
    def add_and_propagate_upper_bound(typ, ast, new_cons = {})
      ...
      @lbounds.each { |lower_t, a|
        if typ.is_a?(VarType) && !typ.lbounds
          QDL::Logging.debug_error :inference, "Nil found in lbounds... Continuing"
          next
        end
        if lower_t.is_a?(VarType) && lower_t.to_infer
          ...
          unless QDL::Type::Type.leq(lower_t, typ, {}, false, ast: ast, no_constraint: true, propagate: true, new_cons: new_cons)
            ...
            raise QDL::Typecheck::StaticTypeError, ("Inconsistent type constraint #{lower_t} <= #{typ} generated during inference.\n #{d1}\n #{d2}")
          end
        end
      }
    end
    ...
    def add_and_propagate_lower_bound(typ, ast, new_cons = {})
      return if self.equal?(typ)
      QDL::Logging.log :typecheck, :trace,  "#{typ} <= #{self}"
      if !@lbounds.any? { |t, a| t == typ }
        QDL::Logging.log :typecheck, :trace,  '@lbounds.any'
        @lbounds << [typ, ast]
        new_cons[self] = new_cons[self] ? new_cons[self] | [[:lower, typ, ast]] : [[:lower, typ, ast]]
      end
      QDL::Logging.log :typecheck, :trace, 'ubounds.each'
      @ubounds.each { |upper_t, a|
        if upper_t.is_a?(VarType) && !upper_t.lbounds
          QDL::Logging.debug_error :inference, "Nil found in upper_t.lbounds... Continuing"
          next
        end
        if typ.is_a?(VarType) && !typ.ubounds
          QDL::Logging.debug_error :inference, "Nil found in ubounds... Continuing"
          next
        end

        QDL::Logging.log :typecheck, :trace, "ubound: #{upper_t}"
        if upper_t.is_a?(VarType)
          ...
          #QDL::Logging.log :typecheck, :trace, "about to check #{typ} <= #{upper_t} with".colorize(:green)

          #QDL::Util.each_leq_constraints(new_cons) { |a, b| QDL::Logging.log(:typecheck, :trace, "#{a} <= #{b}") }

          unless QDL::Type::Type.leq(typ, upper_t, {}, false, ast: ast, no_constraint: true, propagate: true, new_cons: new_cons)
            ...
            raise QDL::Typecheck::StaticTypeError, ("Inconsistent type constraint #{typ} <= #{upper_t} generated during inference.\n #{d1}\n #{d2}")
          end
          #QDL::Logging.log :typecheck, :trace, "Checked #{typ} <= #{upper_t}".colorize(:green)
        end
      }
    end
    ...
    def add_lbound(typ, ast, new_cons = {}, propagate: false)
      ...
      QDL::Logging.log :typecheck, :trace, "#{self}.add_lbound(#{typ}); " + 'propagate'.colorize(:yellow) + " = #{propagate}"
      if propagate
