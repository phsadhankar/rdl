module QDL::Type
  ...
  class MethodType < Type
    ...
    def initialize(args, block, ret, sol=false)
      ...
      state = :required
      args.each { |arg|
        arg = arg.type if arg.instance_of? QDL::Type::AnnotatedArgType
        case arg
        when OptionalType
...
Actual argument type#{args.size > 1 ? "s" : ""}:
        (#{args.map { |arg| QDL::Util.qdl_type_or_class(arg) }.join(', ')}) #{if blk then blk.to_s end}
...
Actual return type:
        #{ QDL::Util.qdl_type_or_class(ret)}
Actual return value:
        #{ ret.inspect }
RUBY
      }
...
#{ [types].map { |t| "        " + t.to_s }.join("\n") }
Actual argument type#{args.size > 1 ? "s" : ""}:
        (#{args.map { |arg| QDL::Util.qdl_type_or_class(arg) }.join(', ')})
Actual argument values (one per line):
#{ args.map { |arg| "        " + arg.inspect }.join("\n") }
RUBY
      }
...
RUBY
    ...
    def block_wrap(slf, inst, types, bind, &blk)
      Proc.new {|*v, &other_blk|
        _, v, other_blk, bind = QDL::Type::MethodType.check_block_arg_types(slf, types, inst, bind, *v)
        tmp = other_blk ? slf.instance_exec(*v, other_blk, &blk) : slf.instance_exec(*v, &blk)
	      tmp = QDL::Type::MethodType.check_block_ret_types(slf, types, inst, tmp, bind, *v)
        tmp
      }
    end
